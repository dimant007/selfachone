...
{
    for( $r = ""; $idx >= 0; $idx = intval( $idx / 26 ) - 1 ) $r = chr( $idx % 26 + 0x61 ) . $r;
    return $r;
}
...
function indicize( $alpha )
{
    ...
    for( $i = 0; $i < $l; $i ++ ) $n = $n * 26 + ord( $alpha[$i] ) - 0x60;
...
function default_columns( $length )
{
    ...
    for( $i = 0; $i <= $length; $i ++ ) $names[$i] = alphacize( $i );
...
class XLSXParser
{
    ...
    {
        $workbook_file = $this->get_tmp_dir() . '/xl/workbook.xml';
        $workbook = simplexml_load_file( $workbook_file );

        $ns_main = 'http://schemas.openxmlformats.org/spreadsheetml/2006/main';
        $ns_relationships = 'http://schemas.openxmlformats.org/officeDocument/2006/relationships';

        $workbook->registerXPathNamespace( 'n', $ns_main );

        if( $sheet = reset( $workbook->xpath( ".//n:sheet[@sheetId={$this->sheetNum}]" ) ) ){
            $reference = $sheet->attributes( $ns_relationships )->id;

            $relations_file = $this->get_tmp_dir() . '/xl/_rels/workbook.xml.rels';
            $relations = simplexml_load_file( $relations_file );
            //var_dump($relations);
            $relations->registerXPathNamespace( 'r', $ns_relationships );
            if( $relation = reset( $relations->xpath( "./*[@Id='{$reference}']" ) ) ){
                if( $target = $relation->attributes()->Target ){
                    return $this->get_tmp_dir() . '/xl/' . $target;
                }
            }
        }
    ...
    protected function apply_headers( $row )
    {
        ...
        if( $this->is_using_default_headers() ){
            foreach( $row as $idx => $row_value ) $composed[alphacize( $idx )] = $row_value;
        }
        else {
            foreach( $this->headers as $idx => $colname ) $composed[$colname] = isset($row[$idx]) ? $row[$idx] : '';
        }
    ...
    protected function set_headers( $arr )
    {
        ...
        if( $this->header_overrides ){
            foreach( $this->header_overrides as $idx => $str ) $headers[column_index( $idx )] = $str;
        }
