...
Class XLSXWriter
{
    ...
    public function writeSheet( array $data, $sheet_name = '', array $header_types = array() )
    {
        ...
        $this->sheets_meta[] = array(
            'filename' => $sheet_filename,
            'sheetname' => $sheet_name,
            'xmlname' => strtolower( $sheet_default ) . ".xml"
        );
    ...
    {
        static $styles = array( 'money' => 1, 'dollar' => 1, 'datetime' => 6, 'date' => 7, 'string' => 0 );
        $cell = self::xlsCell( $row_number, $column_number );
        $s = isset($styles[$cell_format]) ? $styles[$cell_format] : '0';

        if( $cell_format == 'bool' ){
            fwrite( $fd, '<c r="' . $cell . '" s="' . $s . '" t="n"><v>' . ($value ? "1" : "0") . '</v></c>' );
        }
        else {
            if( is_numeric( $value ) && $cell_format == 'number' ){
                fwrite( $fd, '<c r="' . $cell . '" s="' . $s . '" t="n"><v>' . ($value * 1) . '</v></c>' ); //int,float, etc
            }
            else {
                if( $cell_format == 'date' ){
                    if( ! empty($value) ){
                        $value = '<v>' . intval( self::convert_date_time( $value ) ) . "</v>";
                    }
                    fwrite( $fd, '<c r="' . $cell . '" s="' . $s . '">' . $value . '</c>' );
                }
                else {
                    if( $cell_format == 'datetime' ){
                        fwrite( $fd, '<c r="' . $cell . '" s="' . $s . '"><v>' . self::convert_date_time( $value ) . '</v></c>' );
                    }
                    else {
                        if( $value == '' ){
    ...
    protected function writeStylesXML()
    {
        ...
        fwrite( $fd, '<xf applyAlignment="false" applyBorder="false" applyFont="false" applyProtection="false" borderId="0" fillId="0" fontId="0" numFmtId="167" xfId="0"/>' );
        fwrite( $fd, '<xf applyAlignment="false" applyBorder="false" applyFont="false" applyProtection="false" borderId="0" fillId="0" fontId="0" numFmtId="167" xfId="0"/>' );
        fwrite( $fd, '<xf applyAlignment="false" applyBorder="false" applyFont="false" applyProtection="false" borderId="0" fillId="0" fontId="0" numFmtId="60" xfId="0"/>' );
        fwrite( $fd, '</cellXfs>' );
